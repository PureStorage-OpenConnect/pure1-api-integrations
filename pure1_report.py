...
AGGREGATION_TYPE = 'max' #can be set to either 'avg' or 'max'
...
def generate_fleet_report(pure1_api_id, pure1_pk_file, pure1_pk_pwd):
    ...
    with open(report_filename, 'w') as csvfile:
        ...
        filewriter.writerow(['Array Name', 'Model', 'OS Version', 'Total Capacity (TB)', 'Volumes (TB)', 'Snapshots (TB)', 'Shared (GB)', str.format('% Used (Last {} days)', REPORTING_INTERVAL_DAYS), str.format('% Used (Prev. {} days)', REPORTING_INTERVAL_DAYS), 'Provisioned (TB)', '% Provisioned', 'Data Reduction'])
        ...
        for array in arrays:
            ...
            if hasattr(response, 'items'):
                    ...
                    if metric_item.data:
                        ...
                        for metric_data in metric_item.data:
                            ...
                            if metric_name == 'array_total_capacity':
                                ...
                                provisioned_space = round(metric_data[1]/BYTES_IN_A_TERABYTE, 2)
                                provisioned_space_previous = round(compared_metric_data[1]/BYTES_IN_A_TERABYTE, 2)
                ...
                if total_capacity != 0:
                    ...
                    percent_provisioned = round(provisioned_space*100/total_capacity, 1)
                    filewriter.writerow([array.name, array.model, os_version, total_capacity, volume_space, snapshot_space, shared_space, percent_used, percent_used_previous, provisioned_space, percent_provisioned, data_reduction])
            else:
